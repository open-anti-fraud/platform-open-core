...
from django.db.models import FloatField, DateField, Func
...
def resolve_profiles_raw(*args, **kwargs) -> ProfilesCollection:
    ...
    current_extra_fields = p_settings.extra_fields if not created else []
    for profile in profiles:
        extra_fields = set(profile.info.keys()) - set(settings.REQUIRED_PROFILE_FIELDS) - set(current_extra_fields)
        if extra_fields:
            for field in extra_fields:
                del profile.info[field]
