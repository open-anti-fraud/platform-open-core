...
from platform_lib.utils import ApiError
...
class LoginManager:
    ...
    def registration(cls,
        ...
        with transaction.atomic():
            ...
            LabelManager.create_default_labels(str(workspace.id), label_titles=['My persons'])
...
class WorkspaceManager:
    DEFAULT_CONFIG = {
        'is_active': True,
        'template_version': settings.DEFAULT_TEMPLATE_VERSION,
    ...
    @staticmethod
    @cached(cache=LRUCache(maxsize=settings.CACHE_MAXSIZE))
    def get_template_version(workspace_id: str) -> str:
        return Workspace.objects.get(id=workspace_id).config['template_version']
    ...
    def create_workspace(title: str,
                         ...
                         password: Optional[str] = None) -> Workspace:
        if config is None:
