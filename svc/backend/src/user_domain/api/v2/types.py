...
class PlatformInfoType:
    ...
    def template_version(self, info: Info) -> str:
        return self.config.get('template_version')

    @strawberry.field()
    def sample_ttl(self, info: Info) -> str:
        return settings.SAMPLE_TTL

    @strawberry.field()
    def activity_ttl(self, info: Info) -> Optional[str]:
        return settings.ACTIVITY_TTL
    ...
    @strawberry.field()
    def retention_activities_count(self, info: Info) -> str:
        return settings.RETENTION_ACTIVITIES_COUNT
